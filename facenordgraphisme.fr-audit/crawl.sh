#!/bin/bash
# Crawl all known pages of facenordgraphisme.fr, save raw HTML + extract metadata
BASE="https://www.facenordgraphisme.fr"
OUT="facenordgraphisme.fr-audit"
mkdir -p "$OUT/raw"
...
echo "url,status,title,title_len,meta_desc_len,canonical,h1,robots,og_type,has_jsonld,word_count" > "$OUT/inventory.csv"
...
for path in "${urls[@]}"; do
  ...
  word_count=0
  echo "\"$path\",$status,\"$title\",$title_len,$meta_desc_len,\"$canonical\",\"$h1\",\"$robots\",\"$og_type\",$has_jsonld,$word_count" >> "$OUT/inventory.csv"
  sleep 0.3
done
