...
export function Sidebar({ isCollapsed = false, onToggleCollapse }: SidebarProps) {
  ...
  return (
    <>
      ...
      <div className={`fixed inset-y-0 left-0 z-40 bg-white dark:bg-[#111111] shadow-[0_0_40px_rgba(0,0,0,0.03)] transform transition-transform duration-300 ease-in-out flex flex-col border-r border-gray-100 dark:border-white/10
                ...
                <div className="w-full nav-item-anim opacity-0">
                  <div className="relative w-full flex">
                    ...
                    <Link 
                      ...
                      className={`group flex items-center gap-4 py-3.5 rounded-xl transition-[background-color,color,transform] duration-200 ease-[cubic-bezier(0.34,1.56,0.64,1)] capitalize w-full
        ...
        <div className={`p-8 flex flex-col gap-6 text-[#666666] dark:text-[#a1a1aa] ${isCollapsed ? 'items-center px-4' : ''}`}>
          ...
          <div className={`flex gap-4 ${isCollapsed ? 'flex-col items-center' : ''}`}>
            <a href="#" className="flex hover:text-[#239ea0] dark:hover:text-white transition-[background-color,color] p-2 hover:bg-gray-50 dark:hover:bg-white/5 rounded-lg" aria-label="Facebook">
            ...
            <a href="#" className="flex hover:text-[#239ea0] dark:hover:text-white transition-[background-color,color] p-2 hover:bg-gray-50 dark:hover:bg-white/5 rounded-lg" aria-label="Instagram">
      ...
      {/* Mobile Overlay */}
      <div
        className={`md:hidden fixed inset-0 bg-black/40 backdrop-blur-sm z-30 transition-opacity duration-300 ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
        onClick={() => setIsOpen(false)}
      />
    </>
  )
